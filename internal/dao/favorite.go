...
func (r *Redis) Favor(userId uint, videoId uint) (bool, error) {
...
func (r *Redis) CancelFavor(userId uint, videoId uint) (bool, error) {
	key := util.UserFavorKey(userId)
...
func (r *Redis) QueryFavorCnt(videoId uint) (uint, error) {
...
func (r *Redis) IncrFavorCnt(userId uint) (uint, error) {
...
func (r *Redis) DecrFavorCnt(userId uint) (uint, error) {
	key := util.UserFavorCntKey(userId)
	err := r.redis.Expire(key, time.Hour).Err()
	if err != nil {
		return 0, err
...
func (r *Redis) IncrFavoredCnt(videoId uint) (uint, error) {
	key := util.VideoFavoredCntKey(videoId)
	err := r.redis.Expire(key, time.Hour).Err()
	if err != nil {
		return 0, err
...
func (r *Redis) DecrFavoredCnt(videoId uint) (uint, error) {
	key := util.VideoFavoredCntKey(videoId)
	err := r.redis.Expire(key, time.Hour).Err()
	if err != nil {
		return 0, err
